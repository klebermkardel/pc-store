...
function ProductDetails() {
    ...
    if (loading) return (
        <div className="min-h-screen bg-[#080b0f] flex items-center justify-center">
            <p className="text-emerald-500 font-mono text-[10px] tracking-[3px] uppercase">// carregando produto...</p>
        </div>
    );

    if (error) return (
        <div className="min-h-screen bg-[#080b0f] flex items-center justify-center">
            <p className="text-red-500 font-mono text-[10px] tracking-[3px] uppercase">!! {error}</p>
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-[#080b0f]">
            <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-10">
                ...
                <Link
                    to="/"
                    className="inline-flex items-center gap-2 text-gray-600 hover:text-emerald-400 font-mono text-xs tracking-widest uppercase transition-colors duration-200 mb-6 md:mb-10 group"
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12">
                    ...
                    <div className="relative overflow-hidden bg-[#0d1117] border border-white/5">
                        ...
                        <img
                            ...
                            alt={product.name}
                            className="w-full h-64 sm:h-80 md:h-full md:max-h-[480px] object-cover brightness-90"
                        />
                    ...
                    <div className="flex flex-col">
                        ...
                        <h1 className="text-2xl md:text-4xl font-bold text-gray-100 uppercase tracking-wide leading-tight mb-1">
                            {product.name}
                        </h1>

                        {/* Preço + estoque */}
                        <div className="flex flex-wrap items-center gap-4 mt-5 pb-5 border-b border-white/5">
                            <div className="text-2xl md:text-3xl font-bold text-gray-100 tracking-wide">
                        ...
                        {/* Especificações */}
                        <div className="bg-[#0d1117] border border-white/5 relative overflow-hidden mb-6 md:mb-8">
                            ...
                            <ul className="divide-y divide-white/[0.04]">
                                {Object.entries(product.specifications).map(([key, value]) => (
                                    <li key={key} className="flex justify-between items-center px-5 py-3 hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <span className="text-gray-200 text-sm font-medium text-right ml-4">{value}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
                        ...
                        <button
                            ...
                            className="w-full py-3 md:py-4 bg-emerald-500/10 border border-emerald-500/25 hover:bg-emerald-500/20 hover:border-emerald-500 disabled:opacity-30 disabled:cursor-not-allowed text-emerald-400 font-bold text-sm tracking-widest uppercase transition-all duration-200"
                        >
